...
#pragma omp parallel for collapse(2)
        ...
        HighFive::File file("../../model/c10_vgg11_8959.h5", HighFive::File::ReadOnly);
        ...
        // Mean-Pooling
        resize_3d(output, 16, 16, 64, context);
        meanpool_plain(input, output, context, eval);
        resize_3d(input, 16, 16, 64, context);
        copy_pt(input, output);
        ...
        std::cout << "=================\n";
        // First Conv
        update_model_mp(kernel, bias, act);
        ...
        meanpool_cipher(input_cipher, output_cipher, 32, 32, context, pack, enc, eval);
        copy_ct(input_cipher, output_cipher, 16, 16);
        print_ct(input_cipher, 16, 16, 3, 64, type, dec, sk);
        ...
        */
        ...
        std::vector<std::vector<int>> vgg_struct = {{128, 256}, {256, 512}, {512, 512}, {512, 512}};
        std::vector<std::vector<char>> vgg_meanpool = {{'M', '0'}, {'M', '0'}, {'M', '0'}, {'M', '0'}};
        std::vector<std::vector<std::string>> vgg_name = {{"2.0", "2.1", "3.0", "3.1"},
                                                          {"3.2", "3.3", "4.0", "4.1"},
                                                          {"4.2", "4.3", "5.0", "5.1"},
                                                          {"5.2", "5.3", "5.2", "5.3"}};
        ...
        int vgg_ic_2 = 64;
        int vgg_oc_2 = 64;
        int vgg_input_size = 16;
        int vgg_output_size = 16;
        ...
        std::vector<HEaaN::Ciphertext> output_cipher_3(vgg_output_size * vgg_output_size, HEaaN::Ciphertext(context));
        ...
        for(int layer = 0; layer < vgg_struct.size(); layer++) {
                ...
                int batch_size = 64;
                int chnl_batch_size;
                if(vgg_struct[layer][0] * vgg_struct[layer][1] / 32768 < 1) {
                    chnl_batch_size = 1;
                }else{
                    chnl_batch_size = vgg_struct[layer][0] * vgg_struct[layer][1] / 32768;
                }

                bool bitm = (vgg_name[layer][0] == vgg_name[layer][2]) ? true : false; // break in the middle
...
#pragma omp parallel for
                ...
                // for(int b = 0; b < 1; b++) {
                    ...
                    IRA2B(input_cipher_2, vgg_input_size, vgg_input_size, rp_pack_size, 32768/rp_pack_size, is_gpu, context, pack, enc, eval, edcoder);
                    ...
                    // MERGE (many->1)
                    MERGE(output_cipher_2, output_cipher, vgg_output_size, vgg_output_size, vgg_oc_2, rp_pack_size, b * 512, "A", is_gpu, context, pack, enc, eval, edcoder);
                }
...
#pragma omp parallel for
        ...
        output_cipher.shrink_to_fit();
        fc_cipher(input_cipher, output_cipher, fc_weight, fc_bias, 10, context, pack, enc, eval, edcoder, dec, sk);
...
#pragma omp parallel for
            for(int i = 0; i < conv_2 * conv_2; i++) {
                ...
                // std::cout << "ROT RIGHT == " << 64*kocb << std::endl;
